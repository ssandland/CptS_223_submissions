	...
	void percolateDown(unsigned int hole)
	{
		...
		for( ; hole * 2 <= _items.size(); hole = child)
		{
			...
			if ( child != _items.size() && _items[child + 1] < _items[child] )
				child++; // switches to right child of hole it is less than leftchild
			if( _items[child] < tmp ) {
				_items[hole] = _items[child];
				_items[child] = tmp;
				//un comment the line below to get a visual of the value percolating down
				//std::cout << "tmp = " << tmp << " ||  hole = " << hole  << "  ||  L = " << _items[hole * 2] << "  ||  R = " << _items[(hole * 2) + 1] << "  ||  PARENT = " << _items[hole] << std::endl;
			}
			else
				break;
		}
...
public:
	...
	/**
	 * Returns the value at a specific location in the vector
	 * was unable tp access the vector fucntions do to private but this shoudl help
	 */
	int at(int index)
	{
